...
function MessageList({ messages, setMessages, user_id, selectedUser, reloadMessages }) {
    ...
    const handleEditMsg = async (e) => {
        ...
        try {
            const response = await axios.put(`${apiUrl}/messages/edit/${messageIdToEdit}`, { text: messageToEdit, user_id }, { headers: { instabook_token } });
    ...
    return (
        <div className="flex-1 p-4 max-h-screen h-screen bg-gray-800 rounded-lg overflow-y-scroll scrollbar-thin overflow-x-hidden">
            {messages.length === 0 ? (
