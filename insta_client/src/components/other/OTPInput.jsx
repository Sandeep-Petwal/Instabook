...
const OTPInput = ({ email, setStep, loading, setLoading }) => {
    ...
    const handleOtpSUbmit = async () => {
        ...
        try {
            setLoading(true);
            await axios.post(`${apiUrl}/verify/userregistration`, { email, otp: otp.join('') });
            setStep(3);
        } catch (error) {
        ...
        <section className="bg-dark py-10 dark:bg-dark">
            <div className="container">
                <div className='flex justify-center flex-col items-center'>
                    ...
                    <form id="otp-form" className="flex text-white gap-2 justify-center items-center">
                        {otp.map((digit, index) => (
                            <input
                                key={index}
                                type="text"
                                ...
                                className="shadow-xs flex w-16 items-center justify-center rounded-lg border border-gray-600 bg-dark text-white p-3 text-center text-2xl font-medium dark:border-gray-400 dark:bg-gray-800 outline-none sm:text-4xl focus:border-indigo-500 dark:focus:border-indigo-600"
