...
function UserSessions({ user_id }) {
    ...
    const deleteSession = async (sessionId) => {
        try {
            await axiosInstance.delete(`/admin/users/session/${sessionId}`);
            setSessions(
    ...
    const deleteAllSessions = async () => {
        try {
            await axiosInstance.delete(`/admin/users/sessions/${user_id}`);
            setSessions([]);
    ...
    const trimUserAgent = (userAgent) => {
        ...
        for (let i = 0; i < parts.length; i++) {
            if (parts[i].includes("Chrome")) {
                browser = "Chrome"; i
            } else if (parts[i].includes("Firefox")) {
                browser = "Firefox";
            } else if (parts[i].includes("Safari") && !browser) {
                browser = "Safari";
            }

            if (parts[i].includes("Windows")) {
                os = parts[i] + (parts[i + 1] ? " " + parts[i + 1] : "");
            } else if (parts[i].includes("Linux")) {
                os = "Linux";
            } else if (parts[i].includes("Mac")) {
                os = "Mac OS";
            }
        }

        const trimmedUserAgent = `${browser}, ${os}`;
    ...
    return (
        <div className="container mx-auto p-4">
            {/* <Button onClick={() => navigate("/settings")} variant="outline" className="mb-4">
        <ArrowLeft className="h-4 w-4 mr-2" /> Back to Settings
      </Button> */}
            <Card className={`mb-6 ${sessions.length < 1 ? "hidden" : ""}`}>
                <CardHeader>
